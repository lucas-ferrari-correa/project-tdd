...
describe('Property Entity', () => {
  ...
  it('deve lançar um erro se o nome for vazio', () => {
    expect(() => {
      new Property('1', '', 'Descrição', 4, 2000)
    }).toThrow('O nome é obrigatório')
  })

  it('deve lançar um erro se o número de hóspedes for menor do que zero', () => {
    expect(() => {
      new Property('1', 'Nome', 'Descrição', -2, 2000)
    }).toThrow('O número máximo de hóspedes deve ser maior do que zero')

    expect(() => {
      new Property('1', 'Nome', 'Descrição', 0, 2000)
    }).toThrow('O número máximo de hóspedes deve ser maior do que zero')
  })
